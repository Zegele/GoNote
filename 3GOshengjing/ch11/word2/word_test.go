...
func randomPalindrome(rng *rand.Rand) string { //这个函数创造了回文字符串
	...
	return string(runes) //创造了回文字符串
}

func TestRandomPalindromes(t *testing.T) {
	// Initialize a peudo-random number generator.
	seed := time.Now().UTC().UnixNano()
	t.Logf("Random seed: %d", seed)       //日志中记录随机数种子
	rng := rand.New(rand.NewSource(seed)) //什么意思
...
// randomPalindrome returns al palindrome whose length and contents
// are derived from the pseudo-random number generator rng.
